...
-- Ctrl+S = Save + Format (in Normal & Insert mode)
vim.keymap.set({ "n", "i" }, "<C-s>", function()
  -- Save first
  vim.cmd("write")

  -- Then format asynchronously
  require("conform").format({ async = true }, function(_, did_edit)
    if did_edit then
      vim.notify("Saved & formatted", vim.log.levels.INFO)
    else
      vim.notify("Saved (no changes)", vim.log.levels.INFO)
    end
  end)
...
do_now(function()
  ...
  -- Configure rust-analyzer to detect Cargo.toml as root
